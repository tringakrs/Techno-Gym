...
const TodayOrderTable = ({ order }) => {
  return (
    <Fragment>
      <tr className=" border-b border-solid border-b-[#a9a9a940] ">
        <td className=" hover:bg-gray-200 p-2 flex flex-col space-y-1 items-center">
          {order.allProduct.map((item, index) => {
            return (
              <div key={index} className="flex space-x-2">
                <span className="max-w-[190px] break-words ">{item.id.pName}</span>
                {item.quantitiy === 1 ? "" : <span className="font-semibold">{item.quantitiy}x</span>}
              </div>
            );
          })}
        </td>
        ...
        <td className="hover:bg-gray-200 p-2 break-words text-center"><span className="block w-[350px]">{order.address}</span></td>
        <td className="p-2 text-center">
