...
const AddProductDetail = ({ categories }) => {
  ...
  const [fData, setFdata] = useState({
    pName: "",
    pImage: null, // Changed to store the image file object
    pDescription: "",
    pStatus: "Active",
    pCategory: "",
    pPrice: "",
  ...
  const submitForm = async (e) => {
    ...
    if (!fData.pImage) {
      setFdata({ ...fData, error: "Please add an image" });
      setTimeout(() => {
        ...
        >
          ...
          <form className="w-full" onSubmit={(e) => submitForm(e)}>
            ...
            <div className="mt-4">
              <button
                type="submit"
                className="bg-indigo-500 text-white py-2 px-4 hover:bg-indigo-600 rounded focus:outline-none"
              >
                Add Product
              </button>
            </div>
